...
func _ready() -> void:
	...
	_logger.info("health", name, "❤️ initialized mount=%d rider=%d" % [int(mount_hp), int(rider_hp)])
...
func apply_damage(value: float, hit_rider: bool = false) -> void:
	...
	if hit_rider:
		...
		_logger.stat_delta("health", name, "rider_hp", before, rider_hp, "❤️")
	else:
		var beforem := mount_hp
		mount_hp = max(0.0, mount_hp - effective)
		_logger.stat_delta("health", name, "mount_hp", beforem, mount_hp, "❤️")
	emit_signal("health_changed", mount_hp, rider_hp)
